...
# st.session_state.page tells us which page to render right now.
# It starts at 0 (Section 1) and counts up to 5 (Result).

if "page" not in st.session_state:
    st.session_state.page = 0

# Streamlit DELETES widget keys from session_state when the widget isn't
# rendered on the current rerun. That breaks multi-page wizards because
# Page 1 widgets are not rendered when Page 2 is showing. The fix is to
# re-assign every key to itself once per rerun, which "claims" the values
# and prevents the deletion.
...
def restart():
    # Clear every key in session_state so the survey can be retaken cleanly
    for k in list(st.session_state.keys()):
        del st.session_state[k]
    st.session_state.page = 0

# OFFICIAL RESEARCH HEADER (visible on every page)
st.markdown(
...
# Confidentiality and purpose statement - shown only on Page 1
...
# PAGE 1 - SECTION 2 (Awareness)
    ...
    # Q2.4 is MANDATORY - we use index=None so nothing is pre-selected
    st.radio(
    ...
    # Back / Next - block Next if Q2.4 not answered
...
# PAGE 2 - SECTION 3 (Adoption)
    ...
    # Q3.1 is MANDATORY - index=None so nothing pre-selected
    st.radio(
    ...
    # Validate Q3.1 mandatory and selection limits before allowing Next
    col_back, col_next = st.columns(2)
    with col_back:
        st.button("← Back", on_click=go_back)
    with col_next:
...
# PAGE 5 - RESULT (scoring + DARMM heatmap)
    ...
    # Safety check: if the user somehow reached this page without answering
    # the mandatory questions, show a friendly error instead of crashing.
    if st.session_state.get("q2_4") is None or st.session_state.get("q3_1") is None:
        ...
        st.stop()
    # Read every form value from session_state (using .get so
    # questions that were skipped get sensible defaults)
    ...
    # Translate user-friendly text into engine keys
    KNOWLEDGE_LEVEL_MAP = {
    ...
    # Real motivation: anything other than "No specific motivation"
    real_motivations = []
    for m in q3_5:
    ...
    # Real benefits: anything other than "Not applicable"
    real_benefits = []
    for b in q3_6:
    ...
    # Load the 120-MSME dataset and build the 4x5 density matrix.
    # Rows: Digital A-D (A at the bottom of the chart). Cols: LSS L1-L5.
    ...
    # Layer 2: a red dot at the respondent's position
